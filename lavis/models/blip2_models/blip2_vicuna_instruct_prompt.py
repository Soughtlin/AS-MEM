...
from utils import Prompt_Enhence


@registry.register_model("blip2_vicuna_instruct_prompt")
class Blip2VicunaInstruct_MALMM(Blip2Base):
    ...
    def __init__(
        ...
        self.Prompt_Enhence = Prompt_Enhence(
        ...
        if self.qformer_text_input:
            ...
            if self.use_memory_bank:
                ...
                for t in range(T):
                    ...
                    image_embeds_ori = image_embeds  # [B,N,C]
                    ...
                    self.Prompt_Enhence = self.Prompt_Enhence.to(image.device)
                    prompt_visual_enhence = self.Prompt_Enhence(
                    ...
                    prompt_visual_enhence = image_embeds_ori + prompt_visual_enhence  # [B, N, C]

                    prompt_visual_enhence = prompt_visual_enhence.unsqueeze(1)  # [B,1,N,C]

                    if t == 0:
                        encoder_hidden_states = prompt_visual_enhence  # [B, 1, N, C]
                        self.visual_memory_bank = prompt_visual_enhence.detach()  # [B, 1, N, C]
                        self.size_constant = torch.ones(B, 1, N).to(image_embeds.device)  # [B, 1, N]
                        self.compression_size = self.size_constant  # [B, 1, N]
                    else:
                        encoder_hidden_states = torch.cat([self.visual_memory_bank, prompt_visual_enhence],
                                                          dim=1)  # [B, (t+1), N, C]
                        self.visual_memory_bank = torch.cat([self.visual_memory_bank, prompt_visual_enhence.detach()],
                                                            dim=1)  # [B, t+1, N, C]
        ...
        if self.qformer_text_input:
            ...
            if self.use_memory_bank:
                ...
                for t in range(T):
                    ...
                    if t == 0:
                        self.visual_memory_bank = prompt_visual_enhence  # [B, 1, N, C]
                        self.size_constant = torch.ones(B, 1, N).to(image_embeds.device)  # [B, 1, N]
                        self.compression_size = self.size_constant  # [B, 1, N]
                    else:
                        self.visual_memory_bank = torch.cat([self.visual_memory_bank, prompt_visual_enhence],
                                                            dim=1)  # [B, t+1, N, C]
                        self.compression_size = torch.cat([self.compression_size, self.size_constant],
                                                          dim=1)  # [B, t+1, N]
